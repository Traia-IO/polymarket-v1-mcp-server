#!/usr/bin/env python3
"""
Polymarket v1 MCP Server - FastMCP with D402 Transport Wrapper
...
- POLYMARKET_V1_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('polymarket-v1_mcp')
...
API_KEY = os.getenv("POLYMARKET_V1_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  POLYMARKET_V1_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"Polymarket v1 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("Polymarket v1 MCP Server", host="0.0.0.0")
...
# - POLYMARKET_V1_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="polymarket-v1-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "polymarket-v1-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Polymarket v1 MCP Server")
    logger.info("="*80)
